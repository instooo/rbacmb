	...
	public function edit_html($fields,$info){		
		...
		foreach($fields as $key=>$val){
			if($key!=='val'){			
				switch($val[2]){
					case 'input':
						$html.="<tr><td>".$val['1']."</td><td><input type='text' class='manager-input s-input' name='".$val['0']."' id='".$val['0']."' style='width:300px' value='".$info[$val['0']]."'></td></tr>";
						break;
					case 'editor':
						$html.="<tr><td>".$val['1']."</td><td><textarea name='".$val['0']."' id='".$val['0']."' data-type='".$val[2]."'>".$info[$val['0']]."</textarea></td></tr>";
						break;
					case 'text':
						$html.="<tr><td>".$val['1']."</td><td><textarea type='text' class='manager-input s-input' name='".$val['0']."' id='".$val['0']."' style='width:400px; height:100px'>".$info[$val['0']]."</textarea></td></tr>";
						break;
					case 'file':						
						//对图片进行处理	
						if(trim($info[$val['0']],'|')){						
						...
						$htmla="<input type='hidden' class='manager-input s-input' name='".$val['0']."'  style='width:300px' value='".$info[$val['0']]."'>";						
						$html.='<tr><td>'.$htmla.$val['1'].'</td><td><div class="upload-category clearfix">						
						<a href="javascript:void(0);" id="'.$val['0'].'" class="category-y fl">上传预览图</a>
						<span class="category-notice fl" data-tag="preview">支持jpg、png格式，RGB模式，单张（宽、高大于1200px）</span>
						<span class="notice fl" data-name="preview">
					...
					case 'mu_file':
						...
						$htmlb="<input type='hidden' class='manager-input s-input' name='".$val['0']."'  style='width:300px' value='".$info[$val['0']]."'>";						
						$html.='<tr><td>'.$htmlb.$val['1'].'</td><td><div class="upload-category clearfix">						
						<a href="javascript:void(0);" id="'.$val['0'].'" class="category-y fl">上传预览图</a>
						<span class="category-notice fl" data-tag="preview">支持jpg、png格式，RGB模式，单张（宽、高大于1200px）</span>
						<span class="notice fl" data-name="preview">
