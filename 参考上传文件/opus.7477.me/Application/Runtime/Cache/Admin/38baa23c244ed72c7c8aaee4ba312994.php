...
<head>
    ...
    <title>下载列表</title>
    <meta name="keywords" content="下载列表" />
    <meta name="description" content="下载列表" />
...
<!-- header end -->
<div id="wrapper">
    <form id="form" method="get" action="<?php echo U('test/index');?>">
        <input id="pagesize" type="hidden" name="pagesize" value="<?php echo ($pagesize); ?>">
               ...
               <?php echo W('Common/searchType');?>
                    ...
                    <div class="select-list" id="pic_search_type_table">
                        <div id="pic_search_type_list">
                            ...
                            <a pic-click-type="search_type" pic-search-type="vector" href="javascript:void(0);">flv动画</a>
                ...
                <div class="sea-search-m fl search-m">
                    ...
                    <input type="hidden" id="pic_form_search_model" name="search_mode" value="<?php echo ($_GET['search_mode']); ?>"/>
                    <input type="text" class="text fl" name="kw" id="searchinput" placeholder="输入关键字搜索图片" value="<?php echo ($_GET['kw']); ?>" AUTOCOMPLETE="OFF">
              ...
              <i></i>
                ...
                <div class="filter clearfix">
                        ...
                        <div class="row-list-m">
                            <div class="input-m clearfix">
                                ...
                                <form class="new-filter-size">
                                    <input type="text" class="txt fl" onkeyup="this.value=this.value.replace(/\D/g,'')" name="width" onafterpaste="this.value=this.value.replace(/\D/g,'')"  value="<?php echo ($_GET['width']); ?>" placeholder="宽度">
                                    <span class="fl">x</span>
                                    <input type="text" class="txt fl" onkeyup="this.value=this.value.replace(/\D/g,'')" name="height" onafterpaste="this.value=this.value.replace(/\D/g,'')" value="<?php echo ($_GET['height']); ?>" placeholder="高度">
                    ...
                    <div class="row-list fl">
                        ...
                        <input type="text" class="txt fl" value="<?php echo ($_GET['game']); ?>"  name="game" style="padding: 0 10px;height: 26px;line-height: 26px;border: 1px #e4e4e6 solid;-webkit-border-radius: 1px;border-radius: 1px; margin-right: 10px;text-align: center;">
                        <input id="game" type="button" style="width: 48px;height: 30px;color: #fff;background: #269edc;border: none;cursor: pointer;" value="确定">
                    </div>
                    <div class="row-list fl">
                        ...
                        <input id="addtime" type="button" style="width: 48px;height: 30px;color: #fff;background: #269edc;border: none;cursor: pointer;" value="确定">
                    </div>
                </div>
    </form>
</div>
...
<div class="imgshow clearfix">
    <style type="text/css">
        .pagesize{float: left;}
        .pagesize .title { display: block; float: left; width: auto; font-size: 14px; margin: 0 2px; text-align: right; color: #000; vertical-align: middle; padding-top: 4px;}
        .pagesize .msg {display: block; float: left; width: auto; font-size: 14px; margin: 0 2px; text-align: left; color: #000; vertical-align: middle; padding-top: 4px;}
        .pagesize select {display: inline-block; margin: -2px 2px 2px 2px; padding: 3px 0; border: 1px solid #ccc; vertical-align: middle; width: 112px;}
    </style>
    <div class="con">
            <table class="node_table" width="100%">
                <tr>
                    <th><input type="checkbox" class="checkall">编号</th>
                    <th>别名</th>
                    <th>游戏</th>
                    <th>分类</th>
                    <th>尺寸</th>
                ...
                <form id="go-down" action="<?php echo U('media/downLoad');?>" method="get">
                    <?php if(is_array($listinfo)): $i = 0; $__LIST__ = $listinfo;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                            <td><input type="checkbox" name='mids[]' value="<?php echo ($vo["mid"]); ?>"><?php echo ($vo["mid"]); ?></td>
                            <td><?php echo ($vo["alias"]); ?></td>
                            <td><?php echo ($vo["game"]); ?></td>
                            <td><?php echo ($vo["catename"]); ?></td>
                            <td><?php echo ($vo["size"]); ?></td>
                            <td><?php echo ($vo["filesize"]); ?></td>
                            <td><a href="<?php echo ($vo["cps_url"]); ?>" target="_blank"><?php echo ($vo["cps_url"]); ?></a></td>
                            <td><?php echo ($vo["houzui"]); ?></td>
                            <td>
                                <?php echo $vo['is_good']?'<span style="color: red">是</span>':'否'; ?>
                            </td>
                        </tr><?php endforeach; endif; else: echo "" ;endif; ?>
                </form>
            </table>
            <div class="pagesize">
                <label class="title">每页显示：</label>
                <label class="msg">
                  <select id="page" name="pagesize" onchange="gradeChange()">
...
<script>
    ...
    $("select[name='pagesize']").children("option[value='"+pagesize+"']").attr('selected',true);
