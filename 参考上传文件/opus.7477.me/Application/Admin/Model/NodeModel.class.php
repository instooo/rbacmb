...
class NodeModel extends Model{
	...
	public function getNodeListByUid($uid){
		$sql	=	"
		select node.id,node.name,node.title,node.pid,node.level,node.ismenu,node.sort,(node.pid*node.level*node.sort) as sortby from opus_node as node left join opus_access as access on access.node_id	= node.id left join opus_role_user as ru on ru.role_id = access.role_id left join opus_user as user	on 	
		 user.id = ru.user_id 	where user.id	=	$uid ORDER BY sortby DESC
		";
		$rs	=	M('')->query($sql);
		return $rs;
	}
	...
	public function getNodeListByRoleId($roleid){
		$sql	=	"
		select node.id,node.name,node.title,node.pid,node.level,node.ismenu,node.sort,(node.pid*node.level*node.sort) as sortby from opus_node as node left join opus_access as access on access.node_id	= node.id 	where access.role_id	=	{$roleid} ORDER BY sortby DESC
		";
		$rs	=	M('')->query($sql);
		return $rs;
	}
