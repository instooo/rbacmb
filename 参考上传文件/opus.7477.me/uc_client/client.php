...
function uc_user_rename($uid, $newusername) {
	$uid = intval($uid);
	return call_user_func(UC_API_FUNC, 'user', 'rename', array('uid'=>$uid, 'newusername'=>$newusername));
}

function uc_db_connect_close() {
	return call_user_func(UC_API_FUNC, 'user', 'close');
}

function uc_db_connect_open($model) {
	return call_user_func(UC_API_FUNC, 'user', 'open', array('model'=>$model));
}
