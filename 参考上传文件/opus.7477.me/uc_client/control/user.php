...
class usercontrol extends base {
	...
	function onget_user() {
		...
		if($status) {
			return array($status['uid'],$status['username'],$status['email'],$status['password']);
		} else {
			return 0;
		}
	}
	...
	function onrename() {
		...
		$uid = $this->input('uid');
		$newusername = $this->input('newusername');
		$this->db->query("UPDATE ".UC_DBTABLEPRE."members SET username='$newusername' WHERE uid='$uid'");
		return $this->db->affected_rows();
	}
	
	function onclose() {
		return $_ENV['user']->db_connect_close();
	}
	...
	function onopen() {
		...
		$this->init_db();
		$_ENV[$model]->db = $this->db;
		$_ENV['note']->db = $this->db;
	}
}
